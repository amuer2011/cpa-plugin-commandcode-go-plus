...
import (
	...
	plug "cpa-plugin-commandcode-go"
)
...
var abiState = struct {
	...
	plugin *plug.CommandCodeGoPlugin
}{}
...
func CommandCodeGoPluginFree(ptr unsafe.Pointer, len C.size_t) {
	if ptr != nil {
		C.free(ptr)
	}
}
...
func handleRegister(request []byte) ([]byte, error) {
	...
	if p == nil {
		return nil, fmt.Errorf("commandcode-go plugin registration returned invalid capabilities")
	}
...
func currentPlugin() (*plug.CommandCodeGoPlugin, error) {
	abiState.RLock()
...
func callHost[T any](method string, request any) (T, error) {
	...
	code := C.commandcodego_call_host(host, cMethod, requestPtr, C.size_t(len(rawRequest)), &resp)
	if resp.ptr != nil {
		defer C.commandcodego_free_host_buffer(host, resp.ptr, resp.len)
	}
	if code != 0 {
